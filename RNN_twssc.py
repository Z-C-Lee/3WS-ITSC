...
class TWSSC:
        ...
        for epoch in range(1, epochs + 1):
            ...
            with torch.no_grad():  # 不进行梯度计算
                for index, batch in enumerate(valid_iter):
                    ...
                    outputs = self.model(x)
                    loss = criterion(outputs, y)
                    valid_loss += loss.data.item() * x.size(0)  # 累计每一批次损失
    ...
    def one_file_test(self, test_iter, lack, file_num, flag=True):
        ...
        main_test_file = pd.read_csv("data/my_test.csv", header=None,
                                     encoding="utf-8")
        with torch.no_grad():  # 不进行梯度计算
            if flag:
                for index, batch in enumerate(test_iter):
                    if np.array(main_test_file)[index][1] != batch.label[0].item():
                        batch.label = (batch.label - 1) * (-1)
                    ...
                    if ((self.alpha > batch_prob[0][0]) and (batch_prob[0][0] > self.beta)):
                        ...
                        if ((batch.label[0].item() == lack) and (predict_label == lack)):
                            self.current_lack_correct += 1
                            current_lack_correct += 1

                        if(predict_label == batch.label[0].item()):
    ...
    def main(self, num_time):
        ...
        for k in range(num_time):
            ...
            self.model = TextRNN(hidden_size=100, embedding_dim=300, vocab_size=20002, out_size=2) 
            ...
            for trainFile in train_files:
                ...
                REVIEW.build_vocab(train_data, max_size=20000)  # 取出频率最大的前30000个单词
                ...
                if (index != None and len(index) != 0):
                    ...
                    main_test_file = pd.read_csv(self.all_test_filename, header=None, encoding="utf-8")
                    main_test_file = np.array(main_test_file)
                else:
                    break
                ...
                self.one_file_train(optimizer, criterion, train_iter, valid_iter,10)
