...
import numpy as np
import os as os
from miVLAD_MNIST.FunctionTool import load_file
...
class MIL:
    ...
    :param
        filename:   The given MIL data path.
        save_home:  The default temp data save path.
    """
    def __init__(self, path, start, end, save_home="D:/Data/MIL/", bag_space=None):
        self.path = path
        self.start = start
        self.end = end
    ...
    def __init_mil(self):
        ...
        if self.bag_space is None:
            self.bag_space = load_file(self.path, self.start,self.end)
            print(self.bag_space.shape)
        ...
        self.data_name = self.path.strip().split("/")[-1].split(".")[0]
